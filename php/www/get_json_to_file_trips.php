...
// |  Results path     |
// +-------------------+
$json_output_file = "static_json_data_trips.js";
$remote_file = "/www/fst-otm.yurboc.ru/static_json_data_trips.js";

// +-------------------+
// |  Identifiers      |
// +-------------------+
$spreadsheetId = '1RQsv5J4PolYGakte1GIYGtBt2yvPswmWqX7fdkUh-T8';
$range = 'Журнал_импорт!A2:O';
...
if (empty($values)) {
    ...
    foreach ($values as $row) {
        array_push($result_data, array(
          ...
          'book_id'        => array_key_exists( 1, $row) ? $row[ 1] : "",
          'name'           => array_key_exists( 2, $row) ? $row[ 2] : "",
          'club'           => array_key_exists( 3, $row) ? $row[ 3] : "",
          'part_cnt'       => array_key_exists( 4, $row) ? $row[ 4] : "",
          'sport_type'     => array_key_exists( 5, $row) ? $row[ 5] : "",
          'difficulty'     => array_key_exists( 6, $row) ? $row[ 6] : "",
          'region'         => array_key_exists( 7, $row) ? $row[ 7] : "",
          'sub_region'     => array_key_exists( 8, $row) ? $row[ 8] : "",
          'reg_date'       => array_key_exists( 9, $row) ? $row[ 9] : "",
          'start_date'     => array_key_exists(10, $row) ? $row[10] : "",
          'finish_date'    => array_key_exists(11, $row) ? $row[11] : "",
          'full_trip_date' => array_key_exists(12, $row) ? $row[12] : "",
          'mkk_leader'     => array_key_exists(13, $row) ? $row[13] : "",
          'mkk_list'       => array_key_exists(14, $row) ? $row[14] : "",
          ));
    }
}
